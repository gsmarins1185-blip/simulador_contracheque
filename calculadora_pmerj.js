...
// ===== (1) Guarda de sessão (gate de login) =====
// Sem a "pulseira" gravada no login, manda de volta para a tela de login.
// ATENÇÃO: isso roda no navegador, então NÃO é segurança real (burlável
// pelo console). Trava apenas o acesso casual pela URL direta.
try {
  if (sessionStorage.getItem('pmerj_acesso') !== 'ok') {
    window.location.href = 'login.html';
  }
} catch (e) {
  // sessionStorage indisponível (ex.: storage bloqueado): não trava o uso
  console.warn('Gate de sessão: sessionStorage indisponível.', e);
}

// Botão "Sair": remove a pulseira e volta ao login (injetado via JS).
document.addEventListener('DOMContentLoaded', function () {
  ...
  sair.addEventListener('click', function () {
    try { sessionStorage.removeItem('pmerj_acesso'); } catch (e) { /* ignora */ }
    window.location.href = 'login.html';
  });
